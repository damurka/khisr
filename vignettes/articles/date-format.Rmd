...
```
This guide explains how to specify dates and periods for DHIS2's **aggregate**
Analytics API (`get_analytics()`, `get_analytics_by_level()`,
`get_data_sets_by_level()`) — the fixed/relative period codes below are for
the `pe` dimension specifically. `khisr`'s Tracker functions
(`get_tracked_entities()`, `get_events()`, `get_enrollments()`) use plain
ISO-8601 date or datetime strings for their date-range arguments instead
(e.g. `occurred_after = '2024-03-20'`), not the period codes described here —
see [Tracker Data](https://khisr.damurka.com/articles/tracker.html).
